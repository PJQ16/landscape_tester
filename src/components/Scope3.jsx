...
export default function Scope3() {
  ...
  const headActivity = ['Purchased Goods & Services','Capital goods','Fuel and Energy Related Activity Not Included in Scope 1 & 2','Upstream transportation and distribution','Waste generated in operations','Business Travel','Employee Commuting','Upstream leased assets','Downstream transportation and distribution','Processing of sold products','Use of sold products','End -of -life treatment of sold products','Downstream leased assets','Franchises','Investments']
  const cateScope1 = [
    {
      scope: "scope1",
  ...
  const handleInputChange = (event) => {
    ...
    setUserInput((prevUserInput) => {
      const calculatedCO2e =
        value *
        (parseFloat(selectActivity?.CO2 || 0) +
          parseFloat(selectActivity?.Fossil_CH4 || 0))/1000;
      return {
        ...prevUserInput,
        inputValue: value,
  ...
  return (
   ...
   <span className="text-info ps-3">Scope3</span>
    ...
    <div className="accordion-item" key={index}>
      <h2 className="accordion-header">
        <button
          ...
          data-bs-target={`#flush-collapse3Scope${index}`}
          aria-expanded="false"
          aria-controls={`flush-collapse3Scope${index}`}
        >
         {`${index + 1 } ).${activity}`}
        </button>
      </h2>
      <div
        id={`flush-collapse3Scope${index}`}
              ...
              <div className="row mt-4">
                ...
                <table className="table table-hover table-responsive">
                  <thead className="text-center">
                    <tr>
                      <th scope="col">กิจกรรม Scope3</th>
                      <th scope="col">%GHG Scope3</th>
                      <th scope="col">Magnitude</th>
                      <th scope="col">Level of influence
                      (Reduction potential)</th>
                      <th scope="col">Opportunity or Risk</th>
                      <th scope="col"> Sector Guidance	</th>
                  ...
                  <tbody>
                    {headActivity.map((activities,index)=>
                    <tr key={index}>
                      <td scope="row">{`${index + 1} .) ${activities}`}</td>
